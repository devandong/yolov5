...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            # dongxiao: print to compare with tnn
            #filename = "x_"+str(i)+".txt"
            #np.savetxt(filename, x[i].permute(0, 2, 3, 1).numpy().reshape(ny*nx, self.na*self.no), fmt='%.5f', delimiter='\n')
            #print("stride")
            #for v in self.stride:
            #    print(v)
            #print("anchor_grid")
            #for v in self.anchor_grid:
            #    print(v)
            
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            #datalayout: batch, na=3, ny, nx, no=85
            if not self.training:  # inference
                if self.grid[i].shape[2:4] != x[i].shape[2:4]:
                    self.grid[i] = self._make_grid(nx, ny).to(x[i].device)

                y = x[i].sigmoid()
                #grid = self.grid[i].numpy()
                #print(grid)
